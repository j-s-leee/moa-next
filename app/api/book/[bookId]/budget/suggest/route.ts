...
export async function GET(
  ...
  try {
    ...
    const expenseStats = await db
      ...
      .groupBy(expenses.categoryId, categories.id, categories.name, categories.icon, categories.type, categories.expenseType)
    ...
    // 예산 제안 계산
    const suggestions = expenseStats.map((stat) => {
