...
export async function PUT(
  ...
  try {
    ...
    const { categoryId, amount, period, source, incomeType, startDate, endDate, transferredFromBookId } = body
    ...
    if (startDate !== undefined) {
      ...
      updateData.startDate = incomeStartDate
    }
    ...
    if (endDate !== undefined) {
      if (endDate === null) {
        ...
        const incomeEndDate = new Date(endDate)
        ...
        const finalStartDate = updateData.startDate || income.startDate
        if (incomeEndDate < finalStartDate) {
          return NextResponse.json(
            { error: '종료 날짜는 시작 날짜보다 이후여야 합니다.' },
            { status: 400 }
          )
        }
        updateData.endDate = incomeEndDate
      }
    }
