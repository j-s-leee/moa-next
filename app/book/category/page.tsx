...
function CategoryList({
  ...
  onDelete: (id: string) => void;
  onReorder: (
    newOrder: { id: string; name: string; icon: LucideIcon }[]
  ) => void;
}) {
...
export default function CategoryPage() {
  ...
  const handleDelete = (id: string) => {
    ...
    if (confirm("정말 삭제하시겠습니까?")) {
      setPersonalExpenseCategories((prev) => prev.filter((c) => c.id !== id));
      setPersonalIncomeCategories((prev) => prev.filter((c) => c.id !== id));
      ...
      <div className="">
        ...
        >
          ...
          <TabsContent value="expense" className="space-y-6">
            ...
            <div>
              <h2 className="text-sm font-semibold mb-3">공동 카테고리</h2>
              <CategoryList
